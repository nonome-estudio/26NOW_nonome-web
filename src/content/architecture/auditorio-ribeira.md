---
id: arch-auditorio-ribeira
order: 1
title:
  es: Auditorio Ribeira
  en: Ribeira Auditorium
description:
  es: |-
    Equipamiento cultural de escala urbana, sobrio y luminoso.
    Un interior flexible para música, actos y vida pública.
  en: |-
    Urban cultural venue with a sober, luminous presence.
    A flexible interior for music, events, and civic life.
image: /images/projects/auditorio-ribeira.png

tags: ["cultural", "public", "galicia"]
year: 2021
location:
  es: Ribeira, A Coruña
  en: Ribeira, A Coruña
awards: ["Finalista Premios XYZ 2022"]

content:
  - type: text
    markdown: |-
      ## Concept
      Volumen compacto, luz controlada y un interior flexible.

      - Sala principal adaptable
      - Materialidad sobria
  - type: image
    src: /images/projects/auditorio-ribeira.png
    alt: Auditorio Ribeira
    caption:
      es: Imagen principal del proyecto
      en: Main project image
---
